...
func (p *RawPacket) PacketBody() (Packet, error) {
	...
	if err := body.UnmarshalPacketBody(&p.Data); err != nil {
		return nil, err
	}
...
func readPacket(r io.Reader, b []byte, maxPacketLength uint32) ([]byte, error) {
	...
	if int(length) < 5 {
		...
		if int(length) < 0 {
			...
			// and the packet length is longer than math.MaxInt32,
			// thus longer than any possible slice.
...
func (p *RequestPacket) UnmarshalFrom(buf *Buffer) error {
	...
	req, err := newRequestPacketFromType(typ)
	if err != nil {
		return err
	}

	if wrPkt, ok := req.(*WritePacket); ok {
		wrPkt.Data = wrDataPool.Get()
	}
